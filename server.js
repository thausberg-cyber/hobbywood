...
app.get("/",(_,res)=>res.json({service:"look, talk 'n build backend",version:"0.9.15cc",status:"ok"}));
app.get("/health",(_,res)=>res.json({ok:true,version:"0.9.15cc"}));
...
app.post("/talk",async(req,res)=>{try{
  ...
  res.json({reply:d.reply||"Projektstand aktualisiert.",analysis:merged,purchased_parts_updates:Array.isArray(d.purchased_parts_updates)?d.purchased_parts_updates:[],calculation:calc});
...
app.listen(port,()=>console.log(`look, talk 'n build backend 0.9.15cc listening on port ${port}`));
