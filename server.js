...
app.get("/",(_,res)=>res.json({service:"look, talk 'n build backend",version:"0.9.15h",status:"ok"}));
app.get("/health",(_,res)=>res.json({ok:true,version:"0.9.15h"}));
...
app.listen(port,()=>console.log(`look, talk 'n build backend 0.9.15h listening on port ${port}`));
